...
const ProductsModal = ({ id, name, overview, price, count, title, label, endpoint, formInputs }) => {
...
export const ProductsDeleteModal = ({ Id }) => {
  const [loading, setLoading] = useState(false);
  const [isOpen, setIsOpen] = useState(false);
  const handleSubmit = async () => {
    setLoading(true);
    await Backend.call(`/api/dashboard/deleteproduct`, { Id: parseInt(Id) }).then((res) => {
      console.log(res);
    });
    setLoading(false);
    setIsOpen(false);
...
export const ProductsDownloadModal = ({ Id }) => {
  const [loading, setLoading] = useState(false);
  const [isOpen, setIsOpen] = useState(false);
  const handleSubmit = async () => {
    setLoading(true);
    await Backend.call("/api/dashboard/downloadproductfile", { Id: parseInt(Id) }, { isBlob: true });
    setLoading(false);
    setIsOpen(false);
  };
  return (
